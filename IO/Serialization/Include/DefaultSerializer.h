...
#ifndef _IO_SERIALIZATION_INCLUDE_DEFAULTSERIALIZER_
#define _IO_SERIALIZATION_INCLUDE_DEFAULTSERIALIZER_
...
namespace stl { namespace io {
    ...
    class DefaultSerializer: public Mover, public DeserializerBase<Vec<MUInt8>> {
        Vec<MUInt8>  output;

        DefaultSerializer() {}
        DefaultSerializer(Vec<MUInt8>& output): output(output.copy()) { }

      public:
        DefaultSerializer(DefaultSerializer&& obj) {
            this->output = moveObj(obj.output);
        }

        static Fn create(Vec<MUInt8>& vec) -> DefaultSerializer {
            return DefaultSerializer(vec);
        }

        Fn copy() -> DefaultSerializer {
            return DefaultSerializer(this->output);
        }
...
#endif /* _IO_SERIALIZATION_INCLUDE_DEFAULTSERIALIZER_ */
